...
def serve():
    ...
    redistributor = threading.Thread(target=redistrbute, args=(server,))
            ...
            for runner in server.runners:
                ...
                try:
                    response = helpers.communicate(runner["host"],
                                                   ...
                                                   "ping")
                    if response != "pong":
                        print("removing runners %s" % runner)
...
class DispatcherHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        if command == "status":
            ...
            results = command_groups.group(2)[1:].split(":")
            commit_id = results[0]
